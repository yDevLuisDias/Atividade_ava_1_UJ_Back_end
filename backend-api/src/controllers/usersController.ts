...
function isValidRole(value: unknown): value is "student" | "teacher" {
...
export function postUser(req: Request, res: Response): void {
  const payload = req.body as Partial<CreateUserDTO>;
...
export function putUser(req: Request, res: Response): void {
  const id = getRouteId(req);
  if (!id) {
    res.status(400).json({ message: "Parâmetro id inválido." });
    return;
